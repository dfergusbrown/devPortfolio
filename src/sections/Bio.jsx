import React from "react";

const Bio = () => {
  return (
    <section className="bio">
      <h2 className="sectionHead">Bio</h2>
      <article>
        <p>
          This is me, Fergus Brown, a web developer and recent veteran of the US
          Army. As I transition from my job as a linguist, I'm seeking the
          opportunity to build and create whilst cultivating my growing
          skillset. <br/><br/>I am actively seeking opportunities to contribute my diverse
          skill set and passion for web development to innovative projects. If
          you're looking for a dedicated and adaptable team member with a strong
          foundation in technology, language-learning, and discipline gained
          from military service, let's connect.
        </p>
      </article>
    </section>
  );
};

export default Bio;
